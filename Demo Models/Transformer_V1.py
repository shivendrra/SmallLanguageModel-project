...
# hyperparameters
batch_size = 8 # no of independent sequences process in parallel
...
@torch.no_grad() # helps to reduce memory consumption and speed up computations by removing gradients during evolution process
def estimate_loss():
    out = {}
    model.eval()
...
class Block(nn.Module):
    ...
    def __init__(self, n_embd, n_head):
        # n_embd: embedding dimension, n_head: no of heads
        super().__init__()
...
class TransformerLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        x = self.transformer(tok_emb, tok_emb)  # Both src and tgt are the same for language modeling
        x = self.fc(x)
...
# use the same optimizer and training loop
